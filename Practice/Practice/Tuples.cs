...
namespace Practice
{
    class Tuples
    {
        static void Main()
        {
            ...
            var (max, min, avg) = CalculateMax(values);
            Console.WriteLine($"There max is {max} and min is {min} and avg is {avg}");
        ...
        static (double max, double min, double avg) CalculateMax (List<double> num)
        {
            double max, min, avg;
            max = num.Max();
            min = num.Min();
